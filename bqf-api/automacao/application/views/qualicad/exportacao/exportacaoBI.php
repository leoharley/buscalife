    ...
    <div class="row">
        ...
        <div class="col-md-12" id="exportFrm">
            <form action="<?php echo base_url() ?>exportaBI" method="post" enctype="multipart/form-data">
                    ...
                    <label for="dtAtualizacao">Última atualização:</label>
                    <span id="dtAtualizacao" name="dtAtualizacao"><?=  ($infoControleCargaBI[0]->st_iniciar != 1)?(date('d-m-Y', strtotime($infoControleCargaBI[0]->dt_iniciar)) . ' às ' . date('H:i:s', strtotime($infoControleCargaBI[0]->dt_iniciar))):'<span style="color:blue">EM ATUALIZAÇÃO</span>'; ?></span>
                    </div>
                ...
                <!--<div class="form-group">
                    ...
                    <option value="" disabled selected>SELECIONE</option>
                        <?php
                        /*$Id_Empresa = $this->session->flashdata('idempresa');
                        if(!empty($empresasPerfilUsuario))
                        {
                            foreach ($empresasPerfilUsuario as $empresa)
                            {
                                ?>
                            <option value="<?php echo $empresa->Id_Empresa ?>" <?php if ($Id_Empresa&&$empresa->Id_Empresa == $Id_Empresa) { echo 'selected'; } ?>>
                ...
                <span><strong>Solicitando atualização do BI, aguarde...    </strong></span>
                <img src="<?php echo base_url(); ?>assets/images/loading.gif" style="width:100px;height:auto">
                </div>
                <br/>

                <input type="submit" class="btn btn-primary" name="exportSubmit" id="exportSubmit" value="SOLICITAR ATUALIZAÇÃO">
                ...
                <input type="submit" class="btn btn-primary" value="" name="geraTbBI" id="geraTbBI" style="margin-left:5px;visibility: hidden"/>
            <br/>
...
$(document).ready(function () {
        ...
        $('input:file').change(
            ...
            $('#exportSubmit').click(
            function () {
                $("#exportSubmit").toggleClass('disabled');
                $('#loader').show();
            });
    });
</script>