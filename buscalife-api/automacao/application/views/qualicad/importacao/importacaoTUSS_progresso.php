        ...
        <div class="col-md-12" id="importFrm">
            <form action="<?php echo base_url() ?>importaTUSS" method="post" enctype="multipart/form-data">
                ...
                <?php if ($progresso != 'completo') {
                    echo '<div style=\"color:green!important;width:93%!important\" id=\"progressbar\"></div>';
                }
                ?>
                ...
                <?php if ($progresso != 'completo') {
                        ...
                        <div class=\"alert alert-success alert-dismissable\">
                        <button type=\"button\" class=\"close\" data-dismiss=\"alert\" aria-hidden=\"true\">×</button>
                        <strong>O arquivo foi importado com sucesso!</strong><br/>
                        Qtd. Linhas ({$rowcount}) | Inseridos ({$insertcount}) | Atualizados ({$updatecount}) | Não inseridos ({$notaddcount}) | Duplicidades ({$duplicidade})
                        </div>";
                    }
                ?>
                </div>
